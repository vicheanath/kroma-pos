...
class PosDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      // For products table: index id (primary key), name, categoryId, and barcode
      products: "id, name, price, category, categoryId, barcode, stock, description, sku, cost, taxable, taxRate, tags, attributes, variations",

      // For categories table: index id (primary key) and name
      categories: "id, name, description, color, icon",

      // For sales table: index id (primary key) and date for date range queries
      sales: "id, items, total, subtotal, tax, discount, discountType, paymentMethod, date, customerName, customerEmail, customerPhone, notes, receiptNumber",

      // For settings table: index id (primary key)
      settings: "id",
    })
...
export async function initializeDatabase(): Promise<boolean> {
  ...
  try {
    ...
    if (!defaultsInitialized) {
      console.warn("Failed to initialize default settings, but database is open")
    }
    ...
    if (error instanceof Dexie.DexieError) {
      ...
      if (error.name === "VersionError") {
        console.error("Database version conflict. Attempting to delete and recreate database...")
        try {
...
export const salesApi = {
  ...
  getTopSellingProducts: async (limit = 5): Promise<{ productId: string; totalSold: number }[]> => {
    try {
      const db = getDB()
...
export const settingsApi = {
  getReceiptSettings: async (): Promise<ReceiptSettings> => {
    try {
      const db = getDB()
      const settings = (await db.settings.get("receipt_settings")) as ReceiptSettings | undefined
  ...
  saveReceiptSettings: async (settings: Partial<ReceiptSettings>): Promise<void> => {
    try {
      const db = getDB()
  ...
  getAppSettings: async (): Promise<AppSettings> => {
    try {
      const db = getDB()
      const settings = (await db.settings.get("app_settings")) as AppSettings | undefined
